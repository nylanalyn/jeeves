...
class Courtesy(SimpleCommandModule):
    ...
    def __init__(self, bot, config):
        ...
        # This block was missing the essential setup calls. They have been restored.
        self._setup_state()
    ...
    def _cmd_ignore(self, connection, event, msg, username, match):
        target_nick = match.group(1) if match else None
    ...
    def _cmd_stats(self, connection, event, msg, username, match):
        profiles = self.get_state("profiles", {})
        self.safe_reply(connection, event, f"Courtesy stats: {len(profiles)} profiles on record.")
        return True
    ...
    def _set_user_profile(self, username: str, *, title=None, pronouns=None):
        ...
        if pronouns is not None:
            ...
            # If a title is not being explicitly set, infer it from pronouns
            if title is None and "title" not in profile:
                if pronouns == "he/him":
                    profile["title"] = "sir"
