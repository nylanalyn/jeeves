...
class Clock(SimpleCommandModule):
    ...
    def _get_geocode_data(self, location: str) -> Optional[Tuple[str, str, str]]:
        """Fetches geographic coordinates for a location string."""
    ...
    def _cmd_time_self(self, connection, event, msg, username, match):
        ...
        if user_loc:
            ...
            if time_str:
                self.safe_reply(connection, event, f"For {self.bot.title_for(username)}, the time in {user_loc['display_name']} is {time_str}.")
            else:
    ...
    def _cmd_time_other(self, connection, event, msg, username, match):
        ...
        # Handle '!time server' specifically
        ...
        # First, check if the query is a known user
        if target_user_loc:
            time_str = self._get_time_for_coords(target_user_loc['lat'], target_user_loc['lon'])
            if time_str:
                self.safe_reply(connection, event, f"The time for {self.bot.title_for(query)} in {target_user_loc['display_name']} is {time_str}.")
            else:
                self.safe_reply(connection, event, f"I'm afraid I could not determine the timezone for {self.bot.title_for(query)}'s location.")
        # If not a user, treat it as a location
        else:
            geo_data = self._get_geocode_data(query)
            if geo_data:
