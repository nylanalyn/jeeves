...
class Adventure(SimpleCommandModule):
    ...
    def _register_commands(self):
        self.register_command(r"^\s*!adventure\s*$", self._cmd_start,
                              description="Start a choose-your-own-adventure voting session.")
    ...
    def _cmd_start(self, connection, event, msg, username, match):
        ...
        if current and current.get("room") != room:
            ...
            self.safe_reply(connection, event,
                            f"I'm afraid there's already an adventure underway in {other_room}. One expedition at a time, if you please.")
            return True
        ...
        self.safe_reply(connection, event,
                        f"Very good, {title}; an adventure it is. Shall we set out for 1. {options[0]} or 2. {options[1]}? Say 1 or 2 to vote.")
