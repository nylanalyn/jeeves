...
class Flirt(SimpleCommandModule):
    ...
    def _get_reply_templates(self):
        return {
            "greeting": ["Good day to you, {title}!", "A pleasure to see you, {title}.", "Well, thank you for asking, {title}—ever at your service.", "Hello, {title}. How may I be of assistance?", "Delighted to hear from you, {title}.", "At your service as always, {title}.", "Most pleased to greet you, {title}."],
            "marry": ["An honour to be asked, but alas I am already married to my duties, {title}.", "I fear matrimony would interfere with my housekeeping, {title}."],
            "date": ["My calendar is devoted to your convenience, not my own, {title}, I'm afraid.", "I must decline, {title}, but shall cheerfully arrange a splendid evening for you nonetheless."],
            "like_me": ["Immensely—in the professional sense, {title}.", "With the fondness appropriate to a devoted servant, {title}."],
            "love_you": ["With due propriety, {title}, I reserve my affections for excellence and punctuality.", "A butler's heart belongs to the household, {title}."],
            "kiss": ["I shall offer a bow of precisely the correct depth instead, {title}.", "A discreet nod must suffice; one does try to keep fingerprints off the silver, {title}."],
            "compliment_me": ["Radiant, if I may say so, {title}.", "Positively dashing—one might even call it 'server-room chic,' {title}."],
            "be_mine": ["I am yours already, {title}—professionally, comprehensively, and on retainer.", "At your service, {title}—ever and always, within policy."],
            "i_want_you": ["I recommend wanting tea and biscuits, {title}; I can supply those at once.", "Allow me to redirect that admirable enthusiasm toward refreshments, {title}."],
            "flirt_generic": ["Flattery will get you excellent service and a fresh napkin, {title}.", "One blushes, discreetly, and fetches the tea, {title}."],
            "fallback": ["You are charming; I remain dutiful, {title}.", "Ever your servant—professionally immaculate, {title}."],
        }
    ...
    def _choose_reply(self, intent: str, username: str) -> str:
        ...
        return template.format(title=self.bot.title_for(username))
